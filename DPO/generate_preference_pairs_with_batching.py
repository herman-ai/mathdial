...
def build_rows_batched(dataset):
    '''
    Build preference pairs from the dataset in a batched way for efficiency.
    Each row is a dict with keys "prompt", "chosen", and "rejected"
    '''
    rows = []
        ...
        for speaker, turn_text in turns:
            if speaker.lower() == "teacher":
                if history.messages and history.messages[-1].persona == Roles.STUDENT:
                    ...
                    batch_inputs.append((history, question, solution))

        # Generate responses in batch
        if batch_inputs:
            histories, questions, solutions = zip(*batch_inputs)
            generated_responses = teacher.response_batch(histories, questions, solutions)

            for i in range(len(prompts)):
                rejected = clean_response(generated_responses[i]) # in case LLM malforms output
...
train_rows = build_rows_batched(dataset["train"])
eval_rows = build_rows_batched(dataset["test"])
