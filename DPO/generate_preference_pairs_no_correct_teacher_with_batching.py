    ...
    for conv_idx, conversation in enumerate(tqdm(dataset, desc="Building rows", unit="conv")):
        ...
        for speaker, turn_text in turns:
            if speaker.lower() == "teacher":
                if history.messages and history.messages[-1].persona == Roles.STUDENT:
                    prompt = teacher.build_prompt(history, question, solution)
                    # Batched teacher generations for this prompt
                    batch_histories = [history] * num_generations
                    batch_questions = [question] * num_generations
                    batch_solutions = [solution] * num_generations
